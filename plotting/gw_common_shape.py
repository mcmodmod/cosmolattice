...
def main():
    ...
    for sim in sims:
        ...
        kappa = kappa / a[eta]  # * sim.omega_star / sim.H
        mask = kappa <= 4
        k_peak, omega_peak = spectrum_peak(kappa, omega_gw)
        kappa = kappa[mask]
        omega_gw = omega_gw[mask]
        ax.plot(kappa, omega_gw / omega_peak, label=labels[sim])
        # ax.vlines(
        #     [4],
        #     ymin=1e-10,
        #     ymax=1e3,
        ...
        if sim == sims[-1]:
            ax.plot(
                kappa,
                gw_template(kappa, 1.21, 1.43, 0.52),
                color="black",
                label="Fit template",
            )
    ...
    savefile = Path("./figures/common_shape_gw_best_fit.pdf")
