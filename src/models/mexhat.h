...
public:
            ...
            STRINGIFY(MODELLABEL)) // MODELLABEL is defined in the cmake.
  {
    ...
    // input file/command line.  Effectively, by calling
    ...
    // means that we actually expect two values and that we will get an array of
    // double of size two.
    // Contrary to the "initial_amplitudes" parameter and the others above,
    //, the "initial_momenta" is an optional parameter. It can still be
    // specified through
    ...
    // more information on how to fix them.

    setInitialPotentialAndMassesFromPotential();
    // Here we call this function to compute the value of the potential on the
    // homogeneous initial condition  (useful to set the initial Hubble rate).
    // We also compute in this function the masses from the second derivative of
    // the potential evaluated on the homogeneous initial conditions. If you
    // want to do something else, uncomment the next section and do whatever
    // suits your needs.
  ...
  //
  // Now we need to define the physics of the model. We start by defining the
  // potential. We need to specify as  many potential  terms as we specified in
  // the ModelParams, here 2. Then for every potential terms, we define a
  // function
  //' auto potentialTerms(Tag<N>)'  with N =0,...,NPot -1. The type 'Tag<N>'
  // simply allows
  // to define different function with the same name. The 'auto' keyword lets
  ...
  // function.
  {
    ...
    // 'pow<N>(x)'. Works with the known-at-compile-time integer N and compute
    // the expression x*...*x N times. If you don't know the integer at compile
    // time or you don't have an integer, use the more usual syntax pow(x, N).
    // These 'pow' functions are just one example of the many algebraic
  ...
  // not say an array containing  the value of the expression but the expression
  // itself, which can and will be evaluate later on. The type of the expression
  // itself depends on the expression and can be intricated. See manual for more
  // details.
  // - The syntax 0_c is equivalent to Tag<0>(),
  // i.e. creating  an object of type 0. This operator '_c' is a modern C++
  // user-defined type literal, taken from Boost and located in
  // fcn/util/rangeiteration/tagliteral.h .
  ...
  // In exactly the same fashion, we  need to define one derivative of the
  // potential per scalar field (2 in this case).  The integer in Tag<0> tells
  // you the field with respect to which you are defining the derivative of the
  // potential of.
  {
  ...
  // Finally, for the purpose of initializing the masses, the user needs to
  // define in the same fashion the second derivatives of the potential (put
  // 'return 0' if you are not using this feature).
  {
